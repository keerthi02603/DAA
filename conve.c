...
// Function to find and print convex hull edges with indices
void findConvexHull(Point p[], int n) {
    printf("Convex Hull Edges (with indices):\n");
    for (int i = 0; i < n - 1; i++) {
        for (int j = i + 1; j < n; j++) {
            if (onConvexHull(p, n, i, j))
                printf("Edge: (%d, %d) to (%d, %d) | Indices: %d, %d\n",
                       p[i].x, p[i].y, p[j].x, p[j].y, i, j);
        }
    }
}
